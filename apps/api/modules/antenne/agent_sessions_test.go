...
var testMeta = antenneclient.EventMeta{Channel: "agent_session.created", Sender: "Jardin"}

func TestAgentSessionCreatesEntryInMatchingProject(t *testing.T) {
	s := newTestService(t)
	user := seedUser(t, s.orm, "sara@example.com")
	project := seedProject(t, s.orm, "Jardin")

	s.handleAgentSession(sessionEvent(t, "fac_abc", "Jardin", "sara@example.com", "key_1"), testMeta)
...
func TestAgentSessionIdempotentReplay(t *testing.T) {
	...
	seedProject(t, s.orm, "Jardin")

	payload := sessionEvent(t, "fac_abc", "Jardin", "sara@example.com", "key_1")
...
func TestAgentSessionUpsertsByFacileID(t *testing.T) {
	...
	s.handleAgentSession(sessionEvent(t, "fac_abc", "Jardin", "sara@example.com", "key_1"), testMeta)
	s.handleAgentSession(sessionEvent(t, "fac_abc", "Jardin", "sara@example.com", "key_2"), testMeta)
...
func TestAgentSessionSkipsUnknownUser(t *testing.T) {
	...
	s.handleAgentSession(sessionEvent(t, "fac_abc", "Jardin", "nobody@example.com", "key_1"), testMeta)
...
func TestAgentSessionRejectsInvalidTimes(t *testing.T) {
	...
	evt := enveloppe.Event[enveloppe.AgentSession]{
		Payload: enveloppe.AgentSession{
			FacileID: "fac_bad", Project: "Jardin", UserEmail: "sara@example.com",
