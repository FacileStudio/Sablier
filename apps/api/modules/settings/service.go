...
func (service *Service) getSettings(ctx context.Context, userID string) (*Settings, error) {
	...
	return &Settings{WebhookURL: record.WebhookURL}, nil
}

func (service *Service) updateSettings(ctx context.Context, userID string, webhookURL string) (*Settings, error) {
	uid, err := strconv.ParseInt(userID, 10, 64)
	if err != nil {
		return nil, errors.Invalid("invalid user id")
	}
	...
	if err := service.orm.WithContext(ctx).Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{"webhook_url"}),
